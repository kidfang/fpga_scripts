...
fpga_sram_percent=20
fpga_dsp_percent=20
fpga_fabric_percent=20
...
for (( i=1; i<=$fpga_n; i=i+1 ));
	...
	do
		...
		gnome-terminal -t "$fpga_bp" -- $fpga_tool_path/fpga_single_stress.sh $fpga_bp $fpga_sram_percent $fpga_dsp_percent $fpga_fabric_percent
	done
