...
fpga_ini_check=$( lspci | grep -i $fpga_dev | grep -i "rev 80" | wc -l )

if [ $fpga_ini_check -eq 0 ];then
	...
	read -n 1 -p "if all the fpga card can be detect normally Press Enter to next step or Press Ctrl + C to Stop! ..."
	echo -e "\n"

	### Adjusting Power ###
	
	fpga_ini_num=$( lspci | grep -i $fpga_dev | grep -i "rev 01" | wc -l )

	for (( i=1; i<=$fpga_ini_num; i=i+1 ));
        	do
